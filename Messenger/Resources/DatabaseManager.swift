    ...
    */
    ...
    public func createNewConversation(with otherUserEmail: String, name: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        guard let currentEmail = UserDefaults.standard.value(forKey: "email") as? String,
            ...
            print("Failed to get currentEmail and currentName")
            return
        }
        print(currentName)
        ...
        ref.observeSingleEvent(of: .value) { [unowned self] snapshot in
            ...
            self.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value) { [unowned self] snapshot in
        ...
        database.child("\(id)/messages").observe(.value) { snapshot in
            ...
            let messages: [Message] = value.compactMap { dictionary in
                ...
                if type == "photo" {
                    ...
                    let locationItem = Location(location: CLLocation(latitude: latitude, longitude: longitude), size: CGSize(width: 300, height: 300))
    ...
    public func sendMessage(to conversation: String, otherUserEmail: String, name: String, newMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        database.child("\(conversation)/messages").observeSingleEvent(of: .value) { [weak self] snapshot in
            ...
            guard var currentMesages = snapshot.value as? [[String : Any]] else {
                completion(false)
                return
            }
            ...
            currentMesages.append(newMessageEntry)
            
            strongSelf.database.child("\(conversation)/messages").setValue(currentMesages) { error, _ in
                guard error == nil else {
                    completion(false)
                    return
                }
    ...
    /// Deletes a conversation with ID
